 ...
 */
const findSentinels = async (handle, firstOnly) => {
...
const readFuseWire = async (handle, sentinelPos) => {
...
const setFuseWire = async (pathToElectron, fuseVersion, strictlyRequireAllFuses, fuseWireBuilder, fuseNamer) => {
    ...
    try {
        ...
        if (sentinels.length > 2) {
            throw new Error(`Found ${sentinels.length} copies of the fuse sentinel in the provided Electron binary. ` +
        ...
        for (const indexOfSentinel of sentinels) {
            const { version: fuseWireVersion, length: fuseWireLength, wireBytes, wireBytesPos, } = await readFuseWire(handle, indexOfSentinel);
...
export const getCurrentFuseWire = async (pathToElectron) => {
    ...
    try {
        ...
        const { version: fuseWireVersion, length: fuseWireLength, wireBytes, } = await readFuseWire(handle, sentinel);
        const fuseConfig = {
